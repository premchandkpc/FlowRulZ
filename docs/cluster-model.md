...
Round-robin partition assignment per lane. The leader:

1. Counts available lanes (fast/normal/heavy)
2. Assigns partitions round-robin across alive nodes
3. Tracks `partition → node_id` mapping for routing decisions

Partition ownership is used for:
- **Direct publish**: Producer can target a specific partition
- **Reply routing**: Replies route to the node that owns the reply partition
- **State affinity**: Related events hash to same partition → same node
...
7. Leader waits for ACKs from a quorum of followers
...
- Gossip protocol for automatic discovery without seed configuration
- Multi-az failover
- Geo-partitioning
- Canary deployments
