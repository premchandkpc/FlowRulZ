...
use super::{
    dag, emit, gate, map, next, parallel, StepResult,
};
...
impl<'a> VM<'a> {
    ...
    fn dispatch(&mut self, instr: &Instruction) -> Result<(), String> {
        ...
        let result = match instr.op {
            ...
            OpCode::Key | OpCode::Label | OpCode::SvcArg | OpCode::JumpOffset => Ok(()),
    ...
    fn op_jmp(&mut self, instr: &Instruction) -> Result<(), String> {
        ...
        if target >= self.plan.instructions.len() {
            return Err(format!(
                "Jmp target {} out of bounds ({} instructions)",
                target,
...
mod tests {
    ...
    fn test_vm_fallback_after_failure() {
        ...
        let arena = crate::memory::arena::Arena::new();
        let mut vm =
            VM::new(&plan, make_ctx(b"hello"), arena, &mock_failing_caller);
        vm.run().unwrap();
    ...
    fn test_vm_parallel_collect() {
        ...
        let arena = crate::memory::arena::Arena::new();
        let mut vm =
            VM::new(&plan, make_ctx(b"{\"x\":1}"), arena, &mock_caller);
        vm.run().unwrap();
    ...
    fn test_vm_map() {
        ...
        let arena = crate::memory::arena::Arena::new();
        let mut vm =
            VM::new(&plan, make_ctx(b"{\"x\":42}"), arena, &mock_caller);
        vm.run().unwrap();
        assert_eq!(vm.ctx.hop_count, 1);
    }
    ...
    fn test_vm_map_on_any_type_string() {
        ...
        let arena = crate::memory::arena::Arena::new();
        let mut vm =
            VM::new(&plan, make_ctx(b"{\"msg\":\"hello\"}"), arena, &mock_caller);
        vm.run().unwrap();
        assert_eq!(vm.ctx.hop_count, 1);
    }
}
